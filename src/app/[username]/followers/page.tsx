...
export default function FollowersPage() {
  ...
  const followers: Follower[] = [
    ...
    },
    {
      id: '6',
      name: '孙八',
      avatar: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=professional%20avatar%20portrait%20of%20a%20middle-aged%20Chinese%20woman%2C%20clean%20background%2C%20high%20quality&image_size=square',
      bio: '数据分析师，擅长数据可视化',
      followedAt: '1周前',
      isFollowing: false
    },
    {
      id: '7',
      name: '周九',
      avatar: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=professional%20avatar%20portrait%20of%20a%20young%20Asian%20man%2C%20clean%20background%2C%20high%20quality&image_size=square',
      bio: 'DevOps工程师，专注于自动化',
      followedAt: '2周前',
      isFollowing: true
    },
    {
      id: '8',
      name: '吴十',
      avatar: 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=professional%20avatar%20portrait%20of%20a%20young%20Asian%20woman%2C%20clean%20background%2C%20high%20quality&image_size=square',
      bio: '移动开发工程师，专注于React Native',
      followedAt: '3周前',
      isFollowing: false
    }
  ]
      ...
      <main className="flex-1 py-8">
        <div className="container mx-auto px-4">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border border-border mb-8">
            <div className="mb-4">
              <h1 className="text-2xl font-bold text-foreground">粉丝列表</h1>
              <p className="text-sm text-secondary mt-1">查看关注您的用户</p>
            </div>
          </div>
            ...
            <div className="p-6">
              <div className="space-y-6">
                {followers.map((follower) => (
                  <div key={follower.id} className="flex items-start space-x-4 p-4 border border-border rounded-md">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h3 className="text-lg font-semibold text-foreground">{follower.name}</h3>
                        <span className="text-xs text-secondary">{follower.followedAt}</span>
                      </div>
                      <p className="text-sm text-secondary mt-1">{follower.bio}</p>
                      <div className="mt-3 flex space-x-2">
                        <button 
                          className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${follower.isFollowing ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-primary text-white hover:bg-primary/90'}`}
                        >
